...
| 項目 | デフォルト値 | CLI指定 |
|------|-------------|---------|
| タイムアウト（ChatGPT） | 1800秒（固定） | 指定不可（指定した瞬間にエラー） |
| ポーリング間隔 | 5秒（固定） | 指定不可（指定した瞬間にエラー） |
| 待機中のタブ自動リフレッシュ | 600秒ごと（固定、リフレッシュ後に3秒待機） | 指定不可 |
| 進捗表示 | 5秒ごと | - |
...
待機（sleep/timeout）系の引数は追加しない。`--timeout` / `--interval` は指定した瞬間にエラー。
...
| `recover` | 再取得（MD保存） | `python chatgpt_multi.py recover --tab 123` |
| `response` | 表示のみ（即時） | `python chatgpt_multi.py response --tab 123` |

recover は search/search1/chat と同じ待機ロジック（タイムアウト1800秒・ポーリング5秒・待機中は10分ごとにタブリフレッシュ）で取得する。
タイムアウト時は、その時点で取得できた内容をベストエフォートで保存する（warning付き）。

## 実行環境の注意（重要）

- コマンド実行ラッパー（例: エージェントのBash実行機能）が独自のタイムアウト/中断を持つ場合、ブラウザ側で回答が完了していても、CLIプロセスが途中で中断されてMDが `(waiting...)` のまま残ることがある。
- 対策: コマンド実行ラッパー側のタイムアウトを1800秒以上（推奨: 1900秒以上）に設定する（設定できる場合）。実行中は別コマンド実行や別リクエスト送信で中断させない。中断された場合は `recover` で回収する。

### ラッパーで30分待機を「確実化」する方法

ラッパーが途中中断し得る場合、バックグラウンド実行に切り替える（プロセスをラッパーの寿命から切り離す）。

例（バックグラウンド実行）:

```bash
nohup python3 .opencode/skills/chatgpt-parallel-research/scripts/chatgpt_multi.py "Q1" "Q2" "Q3" > /tmp/chatgpt_multi.log 2>&1 &
```

実行後は `python3 .opencode/skills/chatgpt-parallel-research/scripts/chatgpt_multi.py tabs` でタブIDを確認し、必要に応じて `recover --tab <id>` で回収する。
...
```
.opencode/skills/chatgpt-parallel-research/
├── SKILL.md                   ← Skill定義
├── references/
│   └── guide.md               ← この仕様書
├── scripts/
...
python .opencode/skills/chatgpt-parallel-research/scripts/chatgpt_multi.py "質問1" "質問2" "質問3"

# 方法2: scriptsディレクトリに移動して実行
cd .opencode/skills/chatgpt-parallel-research/scripts
...
# Chrome拡張の接続確認
python .opencode/skills/chatgpt-parallel-research/scripts/chatgpt_multi.py status
```
...
| 項目 | ChatGPT |
|------|--------|
| ChatGPTController初期値 | 1200秒（20分） |
| CLI固定タイムアウト | 1800秒（30分） |
| CLI固定ポーリング間隔 | 5秒 |
| WebSocketポート | 9224 |
...
`recover --url` は、指定URLをブラウザタブで開いてから、通常のrecover待機（WebSocket + DOM取得）で回収する。
HTTP直fetch（httpx/html2text等）のフォールバックは行わない。
...
| 2026-01-12 | SPEC.md → references/guide.md に移動・リネーム |
...
| 2026-01-13 | recover --url 対応: 会話URLをブラウザで開いてrecover待機で回収 |
