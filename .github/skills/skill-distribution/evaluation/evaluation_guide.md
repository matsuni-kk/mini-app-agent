...
```bash
# 配布先リポジトリ数を確認
find /Users/matuni__/Desktop -path "*/.github/skills/{skill-name}" -type d | wc -l
```
...
```bash
# 配布元のファイル数
find /Users/matuni__/Desktop/browser-controller-agent/.github/skills/{skill-name} -type f | wc -l

# 配布先のファイル数（サンプル）
find /Users/matuni__/Desktop/{repo}/.github/skills/{skill-name} -type f | wc -l
```
...
```bash
# 全配布先でSKILL.mdが存在するか
find /Users/matuni__/Desktop -path "*/.github/skills/{skill-name}/SKILL.md" -type f | wc -l
```
