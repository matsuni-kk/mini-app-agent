# Subagent Maintenance Next Action Triggers
...
|----|---------|-----------|--------|------|
| T1 | 新規Subagentを作成した | `update_agent_master.py` 実行 | 1 | 3環境同期は必須 |
| T2 | 既存Subagentを更新した | `update_agent_master.py` 実行 | 1 | 3環境同期は必須 |
| T3 | Subagentの`skills:`フィールドに新規Skillを指定した | `skill-maintenance` | 2 | 該当SkillにSubagent Executionセクション追加 |
| T4 | Subagentの`skills:`フィールドのSkillが未存在 | `skill-maintenance` | 1 | Skill新規作成が必要 |
| T5 | CLAUDE.mdのワークフロー索引に未登録 | CLAUDE.md更新後に`update_agent_master.py` | 1 | WF索引との整合性維持 |

## スキップ条件
以下の場合のみ、起動条件を満たしても実行をスキップできる:
- ユーザーが明示的に「agents/*.mdのみ編集」と指定した場合
