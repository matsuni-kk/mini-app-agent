...
```bash
# Grok タブ一覧
python .github/skills/x-automation/scripts/grok_multi.py tabs

# Grok 並列検索（3並列以上推奨）
python .github/skills/x-automation/scripts/grok_multi.py "Q1" "Q2" "Q3"

# DeepThink / DeepSearch
python .github/skills/x-automation/scripts/grok_multi.py "Q1" "Q2" "Q3" --deepthink
python .github/skills/x-automation/scripts/grok_multi.py "Q1" "Q2" "Q3" --deepsearch

# ファイル添付
python .github/skills/x-automation/scripts/grok_multi.py "Q1" "Q2" "Q3" --file /path/to/doc.pdf
```
...
- 例: 3並列3往復
  ...
  python .github/skills/x-automation/scripts/grok_multi.py "Q1" "Q2" "Q3" --turns 3

  # 2ターン目: reply で3つの追加質問を同時指定
  python .github/skills/x-automation/scripts/grok_multi.py reply "Follow1" "Follow2" "Follow3"

  # 3ターン目: 同様に3つ指定
  python .github/skills/x-automation/scripts/grok_multi.py reply "Final1" "Final2" "Final3"
  ```
- 制約:
...
- 再取得（全会話取得・上書き）:
  - `python .github/skills/x-automation/scripts/grok_multi.py recover --tab <tab_id>`
...
- ブリッジサーバー起動確認：
  ```bash
  python .github/skills/browser-controller/scripts/browser_controller.py status
  ```

### 検証手順（最小スモーク）
```bash
# X: ユーザー投稿（フィルタ付き）
python .github/skills/browser-controller/scripts/browser_controller.py \
...
```bash
# ユーザー抽出（デフォルト: 現在タブ）
python .github/skills/browser-controller/scripts/browser_controller.py x_users --limit 200

# 特定タブで抽出
python .github/skills/browser-controller/scripts/browser_controller.py x_users --limit 50 --tab 123456
```
...
```bash
# 下書き作成
python .github/skills/browser-controller/scripts/browser_controller.py x_draft "投稿内容を入力"
```
...
```bash
# DMリスト取得
python .github/skills/browser-controller/scripts/browser_controller.py x_dms --limit 50
```
...
```bash
# 単一ユーザーの投稿収集
python .github/skills/browser-controller/scripts/browser_controller.py x_tweets_to_md elonmusk --max 100

# 複数ユーザーの投稿収集
python .github/skills/browser-controller/scripts/browser_controller.py x_tweets_to_md elonmusk jack --max 200

# 最大件数指定
python .github/skills/browser-controller/scripts/browser_controller.py x_tweets_to_md elonmusk --max 1000 --timeout 60
...
```bash
python .github/skills/browser-controller/scripts/browser_controller.py x_home_to_md --max 100 --timeout 60
```
