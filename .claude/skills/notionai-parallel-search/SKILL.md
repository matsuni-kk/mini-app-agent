...
description: "Browser Controller拡張機能を使ってNotion AIで3並列以上の社内検索・抽出を実行し、結果をMarkdown保存する。複数タブでの並列実行、既存タブの再取得、セッション管理に対応。『NotionAIで検索』『Notion AI 並列検索』『Notion AIで社内ナレッジ検索』『Notion AIでコンテキスト収集』を依頼されたときに使用する。Notion情報の検索時には最優先で利用する。"
---
...
2. 実行:
   ...
   - 接続確認:
     - `python .claude/skills/notionai-parallel-search/scripts/notionai_multi.py status`
   - 3並列以上（search）:
     - `python .claude/skills/notionai-parallel-search/scripts/notionai_multi.py "P1" "P2" "P3"`
     - 生ログ（body全文）の保存を有効化する場合（機微情報が含まれる可能性あり）:
       - `python .claude/skills/notionai-parallel-search/scripts/notionai_multi.py "P1" "P2" "P3" --raw`
   - 単一タブ逐次（search1）:
     - `python .claude/skills/notionai-parallel-search/scripts/notionai_multi.py search1 "P1" "P2"`
   - 既存タブの再取得（全文取得・上書き）:
     - `python .claude/skills/notionai-parallel-search/scripts/notionai_multi.py recover --tab <tab_id>`
   ...
   - マルチターン並列（search後の継続質問）:
     ...
     - 例: 3並列3往復
       ...
       python .claude/skills/notionai-parallel-search/scripts/notionai_multi.py "P1" "P2" "P3" --turns 3

       # 2ターン目: reply で3つの追加質問を同時指定
       python .claude/skills/notionai-parallel-search/scripts/notionai_multi.py reply "Follow1" "Follow2" "Follow3"

       # 3ターン目: 同様に3つ指定
       python .claude/skills/notionai-parallel-search/scripts/notionai_multi.py reply "Final1" "Final2" "Final3"
       ```
     - 制約:
