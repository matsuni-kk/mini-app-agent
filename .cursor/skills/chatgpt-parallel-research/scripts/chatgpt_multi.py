...
class ChatGPTController:
    ...
    # Long-running ChatGPT pages can get stuck; refresh periodically while waiting.
    TAB_REFRESH_INTERVAL = 600  # seconds
    TAB_REFRESH_POST_WAIT = 3   # seconds
    ...
    async def reload_tab(self, tab_id: int = None) -> Dict:
        """タブをリロード"""
    ...
    def _normalize_session_tabs(self, tabs) -> List[Dict]:
        ...
        for t in tabs:
            if isinstance(t, int):
                normalized.append({'id': t, 'url': None, 'topic': None, 'md_path': None})
    ...
    def _save_session_tabs(self, tabs, turns_total: int = 1, turns_current: int = 1) -> None:
    ...
    def _set_session_tab_md_path(self, tab_id: int, md_path: str) -> None:
        for t in self._session_tabs:
            if t.get('id') == tab_id:
                t['md_path'] = md_path
    ...
    async def get_full_conversation(self, tab_id: int = None, attached_files: List[str] = None) -> List[Dict]:
        ...
        Note:
            chatgpt_get_full_conversation コマンドを使用してスクロールしながら全メッセージを収集。
            ChatGPTの仮想スクロールにより、DOMには一部のメッセージしか表示されないため、
            スクロールが必要。
        """
        # 専用コマンドで全会話を取得（スクロールしながら収集）
        result = await self._cmd(type='chatgpt_get_full_conversation', tabId=tab_id)

        if result.get('success') and result.get('messages'):
            messages = result['messages']
            # ファイル引用を除去
            for msg in messages:
                if msg.get('role') == 'assistant' and attached_files:
                    msg['content'] = self._clean_file_citations(msg['content'], attached_files)
            return messages

        # フォールバック: 従来の方法（1メッセージのみ取得可能）
        conversation = []

        # userメッセージを取得
        user_result = await self.get_text('[data-message-author-role="user"]', tab_id)
        if user_result:
            if 'texts' in user_result and user_result['texts']:
                user_messages = user_result['texts']
            elif 'text' in user_result and user_result['text']:
                user_messages = [user_result['text']]
            else:
                user_messages = []
        else:
            user_messages = []
        ...
        # assistantメッセージを取得
        assistant_result = await self._cmd(type='chatgpt_get_response', tabId=tab_id)
        if assistant_result.get('success') and assistant_result.get('response'):
            assistant_messages = [assistant_result['response']]
        else:
            assistant_result = await self.get_text('[data-message-author-role="assistant"] .markdown', tab_id)
            if assistant_result:
                if 'texts' in assistant_result and assistant_result['texts']:
                    assistant_messages = assistant_result['texts']
                elif 'text' in assistant_result and assistant_result['text']:
                    assistant_messages = [assistant_result['text']]
                else:
                    assistant_messages = []
            else:
                assistant_messages = []
        ...
        # ファイル引用を除去
        assistant_messages = [self._clean_file_citations(m, attached_files) for m in assistant_messages]

        # 交互に組み立て
        max_len = max(len(user_messages), len(assistant_messages)) if user_messages or assistant_messages else 0
    ...
    async def send_message(self, message: str, tab_id: int = None, files: List[str] = None, max_retries: int = 3) -> Dict:
        """メッセージを送信（ファイル添付対応、応答は待たない）

        フロー:
        1. ファイルがあれば添付（確認するまでリトライ）
        2. 入力欄の準備を待機
        3. メッセージ入力・送信（確認するまでリトライ）
        ...
        # Step 2: 入力欄の準備を待機
        if not await self._wait_for_input_ready(tab_id, timeout=30):
            return {'success': False, 'error': 'Input field not ready'}

        # Step 3: メッセージ送信（リトライ付き）
        last_error = None
    ...
    async def _attach_file_with_retry(self, file_path: str, tab_id: int, max_retries: int = 3, timeout: int = 30) -> Dict:
        ...
        for attempt in range(max_retries):
            ...
            # ファイル処理完了まで固定待機（DOM検査するとReactがリレンダリングしてキャンセルされる可能性あり）
    ...
    async def _wait_for_attachment_ready(self, tab_id: int, file_name: str, timeout: int = 30) -> bool:
        ...
        start = time.time()
        while time.time() - start < timeout:
            result = await self._cmd(type='chatgpt_check_attachment', tabId=tab_id, fileName=file_name)
            if result.get('ready'):
                return True
    ...
    async def parallel_search(
        ...
        # 3. 質問を送信（ファイル添付も含む）
        print(f"\n[2/4] Sending questions{' with files' if files else ''}...")
        for i, (tid, q) in enumerate(zip(tab_ids, questions)):
            send_result = await self.send_message(q, tid, files=files)
        ...
        async def wait_for_response(tid: int, idx: int, question: str) -> Dict:
            ...
            last_response = ""
            stable_count = 0
            MIN_RESPONSE_LEN = 10   # 最低回答長（短すぎる回答は完了とみなさない）
            STABLE_THRESHOLD = 3   # 安定判定回数（3秒 x 3 = 9秒）
            
            while True:
                elapsed = time.time() - start

                if (time.time() - last_refresh) >= self.TAB_REFRESH_INTERVAL:
                    print(f"  Tab {idx+1}: Refreshing page ({int(elapsed)}s elapsed)...")
                    await self.reload_tab(tid)
                    await asyncio.sleep(self.TAB_REFRESH_POST_WAIT)
                    last_refresh = time.time()
                    stable_count = 0
                    last_response = await self.get_response(tid, attached_files=files) or last_response
        ...
        if turns > 1 or not close_tabs:
            ...
            if remaining > 0:
                ...
                print(f"[Next] Send follow-up: python chatgpt_multi.py reply \"Q1\" \"Q2\" ...")
            else:
                print(f"[Session] Saved {len(tab_ids)} tabs for multi-turn conversation")
                print(f"  Use 'python chatgpt_multi.py reply \"Q1\" \"Q2\" ...' to continue")

        # 7. タブを閉じる（単発かつclose_tabs指定時のみ）
    ...
    async def single_session_search(
        ...
        # 3. 質問送信 + 応答待機（順次、最初の質問でファイル添付）
        print(f"\n[2/4] Sending questions sequentially{' with files' if files else ''}...")
        topic = self._derive_topic_from_questions(questions)
        md_paths = self._init_individual_mds(questions, model, thinking, topic=topic)
        print(f"[Output] Writing to {len(md_paths)} separate MD files:")
        for i, p in enumerate(md_paths):
        ...
        for idx, question in enumerate(questions):
            ...
            # 最初の質問でのみファイル添付
            files_for_this_q = files if idx == 0 else None
            send_result = await self.send_message(question, tab_id, files=files_for_this_q)
            if not send_result.get('success'):
            ...
            while True:
                ...
                if (time.time() - last_refresh) >= self.TAB_REFRESH_INTERVAL:
                    print(f"  Q{idx+1}: Refreshing page ({int(elapsed)}s elapsed)...")
                    await self.reload_tab(tab_id)
                    await asyncio.sleep(self.TAB_REFRESH_POST_WAIT)
                    last_refresh = time.time()
                    stable_count = 0
                    last_response = await self.get_response(tab_id, attached_files=files) or last_response
    ...
    def _get_flow_path(self, filename: str, topic: str = None) -> Path:
        ...
        # アプリ名を追加: Flow/YYYYMM/YYYY-MM-DD/GPT/セッション名
        app_dir = flow_dir / yymm / yyyymmdd / "GPT"
        app_dir.mkdir(parents=True, exist_ok=True)

        if topic:
            topic_dir = app_dir / self._sanitize_topic(topic)
    ...
    def _write_individual_md(self, md_path: str, result: Dict, turn: int = 1) -> None:
        ...
        if turn == 1:
            ...
            with open(md_path, 'a', encoding='utf-8') as f:
                ...
                f.write(f"**Elapsed**: {elapsed.get('elapsed', 0) if isinstance(elapsed, dict) else elapsed:.1f}s\n")
    ...
    @asynccontextmanager
    async def tab_session(self, agent_id: str = None, url: str = 'https://chatgpt.com/'):
        """
        タブセッションのコンテキストマネージャー

        使用例:
            async with ctrl.tab_session('agent-1', 'https://chatgpt.com/') as session:
                await session.send_message("Hello")
                response = await session.get_response()
        """
        if agent_id is None:
            agent_id = f"chatgpt-{uuid.uuid4().hex[:8]}"

        session = ChatGPTTabSession(self, agent_id)
        try:
    ...
    async def recover(self, tab_id: int = None, url: str = None, files: List[str] = None) -> Dict:
        """タブIDを指定して全会話履歴を再取得し、MDファイルに保存する

        recoverは全会話を取得して保存する処理。
        3ターン続いたタブなら3ターン分全部取得する。
        md_pathがセッションにあれば上書き、なければ新規作成。
        """
        # URL指定は非推奨（セッションにmd_pathがないため）
        if url:
            return {
                'success': False,
                'error': 'recover --url is not supported. Use --tab with a valid session tab.',
                'response': '',
                'elapsed': 0,
                'tab_id': None,
                'url': url
        ...
        if tab_id is None:
            return {
                'success': False,
                'error': '--tab is required for recover',
                'response': '',
                'elapsed': 0,
                'tab_id': None
            }
        ...
        saved_md_path = self._get_session_tab_md_path(tab_id)
        create_new = False

        # md_pathがセッションにない場合は新規作成フラグを立てる
        ...
        if not url:
            ...
            if saved_url:
                ...
                await asyncio.sleep(3)
                url = await self._get_open_tab_url(new_id)
                if not url:
                    return {
                        'success': False,
                        'error': 'Reopened tab does not exist',
                        'response': '',
                        'elapsed': 0,
                        'tab_id': requested_tab_id,
                        'url': saved_url
                ...
                if ('chatgpt.com/c/' in saved_url) and ('chatgpt.com/c/' not in url):
                    return {
                        'success': False,
                        'error': 'Conversation does not exist or is not accessible',
                        'response': '',
                        'elapsed': 0,
                        'tab_id': requested_tab_id,
                        'url': url
        ...
        start = time.time()
        generating = await self.is_generating(tab_id)
        while generating and (time.time() - start) < self.timeout:
            print(f"[Recover] Waiting for generation to complete ({int(time.time() - start)}s)...")
            await asyncio.sleep(self.poll_interval)
            generating = await self.is_generating(tab_id)

        # 全会話履歴を取得
        print(f"[Recover] Fetching full conversation...")
        conversation = await self.get_full_conversation(tab_id, attached_files=files)
        elapsed = time.time() - start

        if not conversation:
            return {
                'success': False,
                'error': 'No conversation found in tab',
                'response': '',
                'elapsed': elapsed,
                'tab_id': tab_id
            }

        url_now = await self._get_open_tab_url(tab_id) or url

        # md_pathを決定（新規作成 or 上書き）
        if create_new:
            # 新規作成: topicがなければURLから生成
            if not topic:
                topic = self._derive_topic_from_message(url_now or f"tab_{tab_id}")
            from datetime import datetime
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = f"chatgpt_chat_tab{tab_id}_{timestamp}.md"
            md_path = str(self._get_flow_path(filename, topic=topic))
            print(f"[Recover] Creating new file: {md_path}")
        else:
            md_path = saved_md_path

        # 全会話をMDに書き出し
        self._write_full_conversation_md(md_path, conversation, tab_id, url_now, topic)

        # セッションにmd_pathを保存
        self._update_session_tab_md_path(tab_id, md_path)
        ...
        return {
            'success': True,
            'response': conversation[-1]['content'] if conversation and conversation[-1]['role'] == 'assistant' else '',
            'elapsed': elapsed,
            'tab_id': tab_id,
            'url': url_now,
            'topic': topic,
    ...
    async def _chat_single(self, message: str, tab_id: int, wait: bool = True, files: List[str] = None, topic: str = None) -> Dict:
        ...
        stable_count = 0
        MIN_RESPONSE_LEN = 10
        STABLE_THRESHOLD = 3  # 9秒安定
        
        while True:
            elapsed = time.time() - start

            if (time.time() - last_refresh) >= self.TAB_REFRESH_INTERVAL:
                print(f"[Chat] Refreshing page ({int(elapsed)}s elapsed)...")
                await self.reload_tab(tab_id)
                await asyncio.sleep(self.TAB_REFRESH_POST_WAIT)
                last_refresh = time.time()
                stable_count = 0
...
# ========================================
# ChatGPT Tab Session（並列エージェント対応）
# ========================================

class ChatGPTTabSession:
    """
    ChatGPT専用タブセッション
    ...
    def __init__(self, controller: 'ChatGPTController', agent_id: str):
    ...
    async def start(self, url: str = 'https://chatgpt.com/') -> 'ChatGPTTabSession':
        """新しいタブを作成してセッションを開始"""
        if self._owned:
    ...
    async def acquire(self, tab_id: int, timeout: int = 5000) -> 'ChatGPTTabSession':
        """既存タブのロックを取得"""
        if self._owned:
            raise RuntimeError("Session already started")
    ...
    async def get_response(self, attached_files: List[str] = None) -> str:
        """回答を取得"""
        self._ensure_active()
        return await self._ctrl.get_response(self._tab_id, attached_files)
    ...
    async def set_thinking(self, level: str) -> Dict:
        """Thinking強度を設定"""
        self._ensure_active()
        return await self._ctrl.set_thinking(level, self._tab_id)
    ...
    async def wait_for_response(self, timeout: int = None, attached_files: List[str] = None) -> Dict:
        """
        回答の完了を待機して取得
        ...
        while True:
            ...
            response = await self.get_response(attached_files)
...
Examples:
  ...
  python chatgpt_multi.py search "Q1" "Q2" "Q3" --turns 3

  # 2ターン目以降の送信（セッションタブに並列送信）
...
"""
    ...
    # NOTE: timeout/interval are intentionally NOT configurable.
    # Passing these flags should hard-error (recover is exempt) to keep runs deterministic.
    # Keep them suppressed so they do not appear in --help.
    ...
    parser.add_argument('--turns', type=int, default=1,
                        help='Number of conversation turns (default: 1 = single query)')
    
    # Hard-error if a user tries to control sleeps/timeouts via CLI.
    # This is checked before argparse so we can emit a clearer error.
    forbidden_flags = {
        '--timeout',
        '--interval',
    }
    for a in sys.argv[1:]:
        if a.split('=')[0] in forbidden_flags:
            raise SystemExit(
                "Error: --timeout/--interval は指定できません。"
                "sleep/timeout 系の引数は追加しないでください。待機設定は固定です。"
            )

    args = parser.parse_args()

    # Fixed runtime parameters
    fixed_timeout = 1800  # seconds
    fixed_interval = 3    # seconds
    
    # 既知のコマンド一覧
    KNOWN_COMMANDS = [
        'search', 'search1', 'tabs', 'models', 'thinking', 'set-thinking', 'enable-pro',
        'is-pro', 'set-mode', 'attach', 'chat', 'reply', 'recover', 'response', 'debug-dropdown',
    ...
    # --file と --files を統合（--fileがあればfilesリストに追加）
    if args.file:
        if args.files:
