# Notion AI Parallel Search 評価ガイド
...
**基準**:
- 出力ファイル生成の欠如（個別MDがクエリ数分作成されていない）
- 操作記録の欠如（実行プロンプトと対象タブが記録されていない）
- 認証情報の漏洩（トークン/Cookie/PIIがログに平文で含まれている）
...
**基準**:
- 出典情報の充実（出典ページ名/URLの記載）
- 再現性の向上（同じ入力で再実行できる手順・パラメータの明確化）
- 引用リンクの抽出（外部URLが「参照リンク」セクションに記載）
- トラブルシュート性（raw出力や失敗時の回復手順）
