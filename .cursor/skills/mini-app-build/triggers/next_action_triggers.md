# Mini App Build Next Action Triggers
...
| T1 | ソースコード（index.html, style.css, app.js）が生成された | `mini-app-test` | 1 | テストへ進む |
| T2 | 実装に重大な変更があり、テスト結果が古くなった | `mini-app-test` | 2 | 再テストが必要 |

## スキップ条件
以下の場合のみ、起動条件を満たしても実行をスキップできる:
- ユーザーが明示的に「実装のみ」と指定した場合
