...
2. 実行:
   ...
   - 実行方法:
     - このSkillの `./scripts` に移動して実行:
       - `cd .opencode/skills/chatgpt-parallel-research/scripts && python chatgpt_multi.py "質問1" "質問2" "質問3"`
     - リポジトリルートからフルパス指定で実行:
   ...
   - マルチターン（同じセッションに追加質問）:
    ...
    - 再取得（受け取り側エラー対策）:
      - MD保存あり（タブID）: `python chatgpt_multi.py recover --tab <tab_id>`
        - 生成中でも即座に現在の内容を保存（警告メッセージ付き）
        - 保存先: `Flow/YYYYMM/YYYY-MM-DD/<topic>/chatgpt_chat_tab{id}_{timestamp}.md`
   ...
   - 並列検索オプション（抜粋）:
    ...
    - 制約:
      ...
      - `parallel_search` / `search1` / `chat` のタイムアウトは固定1800秒（30分）で、`--timeout` を指定しても無視される。
      - `recover` / `response` は即座に取得して終了（待機なし）。
