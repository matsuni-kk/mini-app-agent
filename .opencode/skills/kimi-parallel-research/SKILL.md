...
```bash
python3 .opencode/skills/kimi-parallel-research/scripts/kimi_multi.py \
  "質問1" "質問2" "質問3"
```
...
```bash
python3 .opencode/skills/kimi-parallel-research/scripts/kimi_multi.py tabs
```

### 4. モデル一覧確認

```bash
python3 .opencode/skills/kimi-parallel-research/scripts/kimi_multi.py models --tab <tab_id>
```

### 5. マルチターン対話（既存セッションで追加質問）

```bash
python3 .opencode/skills/kimi-parallel-research/scripts/kimi_multi.py chat "追加の質問"
```

### 6. 再取得（既存タブから回答を取得）

```bash
python3 .opencode/skills/kimi-parallel-research/scripts/kimi_multi.py recover
```
