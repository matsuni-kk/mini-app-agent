...
description: "Browser Controller Chrome拡張機能を使って任意のWebサイトをプログラムから操作する汎用スキル。「ブラウザ操作」「Webサイト操作」「DOM操作」「スクレイピング」「browser control」「web automation」を依頼されたときに使用する。"
---
...
2. 実行:
   ...
   - 実行方法:
     - `cd .opencode/skills/browser-controller/scripts && python browser_controller.py <command>`
     - `python .opencode/skills/browser-controller/scripts/browser_controller.py <command>`
